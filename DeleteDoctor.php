...
<head>
    ...
    <title>Delete Doctor - WeCare Admin</title>
    ...
    <style>
        ...
        .delete-header::before {
            ...
            right: 0;
            bottom: 0;
            background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><pattern id="delete-grid" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="rgba(255,255,255,0.05)" stroke-width="1"/></pattern></defs><rect width="100" height="100" fill="url(%23delete-grid)"/></svg>');
            opacity: 0.3;
        }
        ...
        .card-header::before {
            ...
            right: 0;
            bottom: 0;
            background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><pattern id="delete-pattern" x="0" y="0" width="15" height="15" patternUnits="userSpaceOnUse"><circle cx="7.5" cy="7.5" r="2" fill="rgba(255,255,255,0.1)"/></pattern></defs><rect width="100" height="100" fill="url(%23delete-pattern)"/></svg>');
            opacity: 0.3;
        }
    ...
    <div class="delete-container">
        <div class="delete-header">
            <div class="delete-content" style="position: relative; z-index: 2;">
                <h1><i class="fas fa-user-times"></i> Delete Doctor</h1>
                <p>Remove doctors from the system</p>
            </div>
        </div>
        ...
        <div class="delete-content">
            ...
            <div class="delete-card">
                <div class="card-header">
                    <h1><i class="fas fa-exclamation-triangle"></i> Doctor Removal</h1>
                    <p>Select a doctor to permanently remove from the system</p>
                </div>
                ...
                <div class="card-body">
                    <div class="warning-banner">
                        ...
                        <div class="warning-content">
                            ...
                            <p>Deleting a doctor is permanent and cannot be undone. This will remove all associated appointments and clinic assignments. Please ensure you want to proceed with this action.</p>
                        </div>
                    </div>
                    ...
                    <div class="delete-options">
                        ...
                        <div class="delete-method">
                            <div class="method-header">
                                ...
                                <h3 class="method-title">Delete by Doctor ID</h3>
                            </div>

                            <form method="post" action="deletedocfromdb.php">
                                <div class="form-group">
                                    <label class="form-label">Enter Doctor ID (DID)</label>
                                    <input type="number" class="form-control" name="did" placeholder="e.g., 101" required>
                                </div>
                        ...
                        <div class="delete-method">
                            <div class="method-header">
                                ...
                                <h3 class="method-title">Delete by Doctor Name</h3>
                            </div>

                            <form method="post" action="deletedocfromdb.php">
                                <div class="form-group">
                                    <label class="form-label">Select Doctor</label>
                                <select name="doctorname" class="doctor-select" required>
                                    <option value="">--- Select Doctor ---</option>
                                    <?php
                                    require_once('DBconnect.php');
                                    $doctor_result = $conn->query('SELECT * FROM doctor ORDER BY name ASC');

                                    if ($doctor_result->num_rows > 0) {
                                        while($row = $doctor_result->fetch_assoc()) {
                                            echo "<option value='" . $row["DID"] . "'>Dr. " . htmlspecialchars($row["name"]) . " (DID: " . $row["DID"] . ")</option>";
                                        }
                                    } else {
                    ...
                    <?php
                    // Display current doctors list for reference
                    ...
                    if ($doctor_count > 0) {
                        ...
                        echo '<h4 style="color: var(--dark-blue); margin-bottom: 1rem;"><i class="fas fa-users"></i> Current Doctors (' . $doctor_count . ')</h4>';
                        ...
                        while($doctor = $doctors->fetch_assoc()) {
                            ...
                            echo '<div>';
                            echo '<strong style="color: var(--primary-blue);">Dr. ' . htmlspecialchars($doctor['name']) . '</strong>';
                            echo '<br><small style="color: var(--medium-gray);">' . htmlspecialchars($doctor['specialisation']) . ' | ' . htmlspecialchars($doctor['region']) . '</small>';
                            echo '</div>';
                            echo '<span style="background: var(--primary-blue); color: white; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.8rem;">DID: ' . $doctor['DID'] . '</span>';
                            echo '</div>';
                        }
                        echo '</div></div>';
                    }
